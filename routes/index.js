...
const stripe = require("stripe")(process.eventNames.STRIPE_PRIVATE_KEY);
...
//GET - add a product to the shopping cart when "Add to cart" button is prssed
router.get("/add-to-cart/:id", async(req, res) => {
    ...
    try{
        ...
        let cart;
        if((req.user && !req.cart && req.session.cart) || (!req.user && req.session.cart)){
        ...
        if(itemIndex > -1){
            // if product exits in the cart, update the quantity
            cart.items[itemIndex].qty++;
            cart.items[itemIndex].price = cart.items[itemIndex]*product.price;
            cart.totalQty++;
        ...
        req.redirect(req.header.referer);
    }catch (err){
...
router.get("/shopping-cart", async(req, res) => {
    try{
        //find the cart, whether in session or db based on the user state
        let cart_user;
        if(req.user){
            cart_user = await Cart.findOne({user: req.user._id});
        }
        // if user is signed in and has cart, load users cart from the db
        if(req.user && cart_user){
            req.session.cart = cart_user;
            return res.render("shop/shopping-cart", {
                cart: cart_user,
                pageName: "Shopping Cart",
                product: await productsFromCart(cart_user),
            });
        }
        ...
        // otherwise, load the sessions cart
        return res.render("shop/shopping-cart", {
...
router.get("/reduce/:id", async function(req, res, next) {
    ...
    try{
        ...
        if(itemIndex > -1){
            ...
            cart.totalQty--;
            cart.totalCost-=product.price;
            //if the items qty reches 0, remove it from the cart
            ...
            if(cart.totalQty <= 0){
                ...
                await Cart.findByIdRemove(cart._id);
            }
        }
...
//GET: remove all instance of a single product from the cart
router.get("/removeAll/:id", async function(req, res, next){
    ...
    try{
        ...
        if(cart.totalQty <= 0){
            req.session.cart = null;
            await Cart.findByIdAndRemove(car._id);
        }
        res.redirect(req.headers.referer);
    }catch(err){
        console.log(err.messgae);
...
router.get("/checkout", middleware.isLoggedIn, async (req, res)=>{
    ...
    //load the cart with the sessions cart if from the db
