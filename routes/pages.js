...
router.post("/contact-us", [userContactUsValidationRules(), validateContactUs],
(req,res)=>{
    ...
    const mailOpts = {
        ...
        html: `
        <div>
            <h2 style="color: #478ba2; text-align:center;">Clients Name: ${req.body.name}</h2>
            <h3 style="color: #478ba2;">Clients email: (${req.body.email})</h3>
        </div>
        <h3 style="color: #478ba2;"> Clients message: </h3>
    ...
    smtpTrans.sendMail(mailOpts, (error, response)=>{
        if(error){
            req.flash(
                "error",
                "An error occured ... Please check your internet connection ans try agin later"
            );
